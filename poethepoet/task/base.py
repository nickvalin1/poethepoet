...
class TaskSpecFactory:
    ...
    def get(
        ...
        if task_def:
            # This is probably a subtask will be cached by the parent task_spec
            if not task_type:
                task_type = PoeTask.resolve_task_type(task_def, self.config)
                assert task_type
            return self.create(task_name, task_type, task_def, parent=parent)
    ...
    def load(self, task_name: str):
        # TODO: how to track which config file this task came from?

        task_def, config_partition = self.config.looup_task(task_name)
        if task_def is None:
        ...
        if not task_type:
            raise ConfigValidationError(
                "Task definition must include exactly one task key of "
                f"{set(PoeTask.get_task_types())!r}",
                task_name=task_name,
            )
    ...
    def get_task_spec(
        ...
        if task_def:
            if not task_type:
                task_type = PoeTask.resolve_task_type(task_def, self.config)
                assert task_type
            return self.specs.create(task_name, task_type, task_def)
...
class PoeTask(metaclass=MetaPoeTask):
    ...
    class TaskOptions(PoeOptions):
        ...
        capture_stdout: Optional[Union[str, bool]] = None
    ...
    class TaskSpec:
        ...
        parent: Optional["PoeTask.TaskSpec"] = None
        source: Optional["ConfigPartition"]
        ...
        def _base_validations(self, config: "PoeConfig", task_specs: TaskSpecFactory):
            ...
            if self.options.deps:
                for dep in self.options.deps:
                    ...
                    if task_specs.get(dep_task_name).options.get("use_exec"):
                        raise ConfigValidationError(
    ...
    def _get_upstream_invocations(self, context: "RunContext"):
        ...
        if self.__upstream_invocations is None:
            env = context.get_task_env(None, options.envfile, options.get("env"))
