...
class ProjectConfig(ConfigPartition):
    class ConfigOptions(PoeOptions):
        ...
        include: Union[str, Sequence[str], Mapping[str, str]] = tuple()
...
class PoeConfig:
    cwd: Path  # TODO: let project config be source of truth for this?
    ...
    def __init__(
        ...
        self._project_config = ProjectConfig(
            {"tool.poe": table or {}}, cwd=self.cwd, strict=False
        )
    ...
    def task_names(self) -> Iterator[str]:
        yield from {
            ...
            *self._project_config.get("tasks", {}).keys(),
        }
